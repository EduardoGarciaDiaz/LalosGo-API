...
describe('Producst API Succes Cases', () =>{
    let authToken = ''
    let adminUser = null
    ...
    describe('Succes action for Productsendpoint', () => {
        ...
        it('Edit Product data', async () => {
            ...
            expect(res.body.message).toEqual("Producto actualizado exitosamente.")      
        })

        
        it('Edit Product quaintity in branch', async () => {
            let newQuantity = 40
            const res = await request(app)
                .put(`/api/v1/products/${productTest._id}`)
                .send({                    
        ...
        it('Edit Product status to inactive', async () => {
            const res = await request(app)
                ...
                .set('Authorization', `Bearer ${authToken}`)
            expect(res.statusCode).toEqual(200)
            expect(res.body.message).toEqual("Estado del producto actualizado exitosamente")      
        })

        it('Edit Product status to active', async () => {
            const res = await request(app)
                .patch(`/api/v1/products/${productTest._id}`)
                .send({                    
                   newStatus: true
        ...
        it('Get Products', async () =>{
            ...
            expect(res.body.products[0]._id).toEqual(productTest._id)
            expect(res.body.message).toEqual("Productos recuperados exitosamente")  
        })

        it('Get Products by branch', async () =>{
            const res = await request(app)
                .get(`/api/v1/products/${branchTest._id}`)
                .send()
                .set('Authorization', `Bearer ${authToken}`)
            expect(res.statusCode).toEqual(200)
            expect(res.body.branch.branchProducts).toBeDefined()
            expect(res.body.branch.branchProducts[0].product._id).toEqual(productTest._id)
            expect(res.body.message).toEqual("Productos de la sucursal recuperados exitosamente")  
        })

        it('Get Products by branch and category', async () =>{
            const res = await request(app)
                .get(`/api/v1/products/${branchTest._id}/${categoryTest._id}`)
                .send()
                .set('Authorization', `Bearer ${authToken}`)
            expect(res.statusCode).toEqual(200)
            expect(res.body.branch).toBeDefined()
            expect(res.body.branch[0].product._id).toEqual(productTest._id)
            expect(res.body.message).toEqual("Productos de la sucursal recuperados exitosamente")  
        })
